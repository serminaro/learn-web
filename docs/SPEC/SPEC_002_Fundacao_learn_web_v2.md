...
supersede: SPEC-002 v1
referencia: SPEC-001, SPEC-003, DEC-001
---

# SPEC-002 · Fundação do projeto `learn-web` (v2)
...
>
> **Esta v2 supersede a SPEC-002 v1** e só entra em vigor quando aprovada pelo autor. Depende da aprovação da **DEC-001** (inclusão de deploy em produção remota no escopo). Enquanto a DEC-001 ou esta v2 permanecerem `proposto`, vale a v1 e nenhum deploy é executado sob o `learn-web`. A única mudança material frente à v1 é a entrada do **deploy em produção remota** no escopo (§3.1, §3.2, §4, §6); o restante é idêntico à v1.
...
**Deploy em produção remota (novo na v2, cf. DEC-001).** O projeto também entrega o **site publicado em produção remota no Google Cloud**, tratado como **capacidade transversal** exercitada a partir do artefato estático da Camada 1 — não como uma quinta camada. O aprendizado de hospedagem (projeto/billing na nuvem, armazenamento, HTTPS, DNS) acompanha as camadas, e a escolha concreta da plataforma de hospedagem fica registrada em **DEC-002** (a produzir). Esta entrega **não** altera os critérios de fechamento das Camadas 1–4 (§5).
...
- **DevOps avançado** (Docker, Kubernetes, CI/CD, pipelines automatizadas de deploy). O deploy admitido no escopo (§3.1) é o **manual e simples** do site (envio de arquivos estáticos para hospedagem gerenciada); automação de entrega contínua fica fora e exige DEC própria.
- **Deploy do backend dinâmico (PHP) em produção remota.** A inclusão de deploy no escopo (DEC-001) cobre, por ora, o **site estático**. Hospedar o PHP da Camada 3+ remotamente exige serviço de container/dinâmico (ex.: Cloud Run, App Engine) e será objeto de DEC dedicada.

> **Nota de versão.** Na v1 desta SPEC, "Deploy em produção remota" constava integralmente como fora de escopo. A DEC-001 reverteu essa fronteira para o caso do site estático, preservando como fora de escopo a automação avançada (DevOps) e o deploy do backend dinâmico.
...
| Camada | Propósito | Tecnologias | Estado em 2026-06-14 |
|---|---|---|---|
| **C1 — Estrutura e estilo estáticos** | Aprender HTML semântico e CSS moderno construindo páginas estáticas do site. | HTML5, CSS3 (Flexbox, Grid, custom properties), responsividade | Em andamento (páginas home/sobre/contato criadas) |
...
**Deploy como capacidade transversal (v2, cf. DEC-001).** A hospedagem remota **não constitui uma quinta camada** e não tem critério de fechamento próprio na §5. É exercitada sobre o artefato da camada já estável (a partir de C1) e documentada em GUIDE dedicado quando o primeiro deploy for executado. Decisões sobre plataforma e serviço entram como DEC (DEC-002 em diante).
...
> **Sobre o deploy (v2).** O deploy em produção remota é capacidade transversal (§3.1, §4) e **não** tem critério de fechamento numerado nesta seção, por decisão explícita da DEC-001. Sua verificação dá-se por GUIDE de deploy + URL pública funcional, sem alterar C1–C4.
...
- **IA (Claude) disponível como co-autor** sob os controles da SPEC-003 §4.
- **Conta Google Cloud com billing ativo e `gcloud` CLI instalada (novo na v2, cf. DEC-001).** Necessária para a capacidade de deploy do §3.1. O autor opera atualmente em trial com créditos; custos de hospedagem do site estático são desprezíveis. A escolha de plataforma/serviço fica em DEC-002.
...
| **R-FUN-07** | O deploy em produção remota (§3.1) só é executado após DEC-001 aprovada, e a escolha de plataforma é justificada em DEC (DEC-002+). Deploy sem DEC de plataforma é dívida documental. | Cruzamento entre estado do deploy, `docs/DEC/` e GUIDE de deploy | Média |
...
| 2026-06-14 | SPEC-002 v2 rascunhada pela IA em status `proposto`, aguarda revisão do autor. Supersede a v1. Única mudança material: entrada do deploy em produção remota no escopo, conforme DEC-001 (§3.1, §3.2, §4, §6, R-FUN-07). Depende da aprovação da DEC-001. |
